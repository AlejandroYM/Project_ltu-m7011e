// services/recipe-service/fixImages.js
// Corrige URLs de imágenes rotas o incorrectas en MongoDB
// Ejecutar: kubectl exec -it -n chefmatch deployment/recipe-service -- node /app/fixImages.js
...
// Mapa de correcciones: nombre exacto → nueva imageUrl
const imageFixes = {
  // ── IMÁGENES ROTAS (no cargan) ────────────────────────────────────────────
  ...
  // ── IMÁGENES INCORRECTAS (cargan pero no corresponden a la receta) ─────────
  ...
  try {
    ...
    for (const [name, imageUrl] of Object.entries(imageFixes)) {
      ...
      if (result.matchedCount > 0) {
        console.log(`  ✅ Fixed: ${name} (${result.matchedCount} doc${result.matchedCount > 1 ? 's' : ''})`);
    ...
    console.log(`\n📊 Done: ${fixed} recipes updated, ${notFound} not found in DB.`);
    process.exit(0);
  } catch (err) {
